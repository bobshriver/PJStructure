...
NPops<-max(agedataall20$Dataset)
Nyears<-400
...
bayespoint<-cbind.data.frame(rep(years,NPops), c(sitemeanB))
colnames(bayespoint)<-c("Time",'brate')
allmean<-apply(apply(bout.use,c(1,2),mean,na.rm=T),2,median,na.rm=T)
allmeanupper<-apply(apply(bout.use,c(1,2),mean,na.rm=T),2,quantile,prob=c(0.975),na.rm=T)
allmeanlower<-apply(apply(bout.use,c(1,2),mean,na.rm=T),2,quantile,prob=c(0.025),na.rm=T)
...
RelCount[i-1]<-mean(s1$RelB[which(s1$Time>=bins20[i-1] & s1$Time<bins20[i])],na.rm=T) ##mean amount of establishment in each 20 year interval across all data sets
}
...
plot(bayespoint$Time,bayespoint$brate,xlim=c(1600,2000),ylim=c(0,.6), ylab=expression("Establishment rate (20yrs"^-1~")"), xlab='Year (CE)',col=alpha('black',0.6),pch=19, cex=0.5)
...
plot.segmented(s2.seg,add=TRUE,lwd=3, col='coral',lty=2262,psi.lines=TRUE)
...
####Figure 3#####
#Containers
bratio<-array(NA,c(3000,20,NPops))
Bval<-array(NA,c(3000,20,NPops))
Bpredval<-array(NA,c(3000,20,NPops))
...
for (i in 1:3000){
  ...
  for(p in 1:NPops){
    ...
    lampred<-sout.use[i,post1600,p]+mean(bout.use[i,post1600,p],na.rm=T)###Calculate lambda estimates survival + mean establishment 
    ...
    Bpred=(head(Npred,-1)*mean(bout.use[i,post1600,p],na.rm=T))*rev(cumprod(rev(na.omit(sout.use[i,post1600,p])))) ###Same as above, but with mean estab. rate
    Bval[i,(miny:stime[p,2])-17,p]<-B/sum(B) #Relativize to be proportion of total
    Bpredval[i,(miny:stime[p,2])-17,p]<-Bpred/sum(Bpred)
    
    bratio[i,(miny:stime[p,2])-17,p]<-((B/sum(B))-(Bpred/sum(Bpred)))/(Bpred/sum(Bpred)) ###Relative diff. (Changing-constant/constant)
    
    
  }  
  
}
ratiomean<-apply((apply((bratio),c(1,2),mean,na.rm=T)),2,median,na.rm=T)
ratiomeanupper<-apply((apply((bratio),c(1,2),mean,na.rm=T)),2,quantile,prob=c(0.975),na.rm=T)
ratiomeanlower<-apply((apply((bratio),c(1,2),mean,na.rm=T)),2,quantile,prob=c(0.025),na.rm=T)
sitecor<-numeric(NPops)
for(i in 1:NPops){sitecor[i]<-cor(apply(Bpredval,c(2,3),median,na.rm=T)[,i], apply(Bval,c(2,3),median,na.rm=T)[,i], use='pairwise.complete.obs')}
...
legend(1590, .18, legend=c("Changing rate","Constant rate"),
       col=c('coral','steel blue'), cex=0.75,pch=15) 
fig_label("A",region="plot", cex=2) 

text(x=1780,y=.08,"Changing rate",cex=0.75)
text(x=1715,y=.19,"Constant rate",cex=0.75,srt=90)

par(mar = c(4.5,5,1.5,2.5))
matplot(seq(1600,1980,20),apply(bratio,c(2,3),median,na.rm=T),type='l',col=spcol[match(specieslist,species)],lty=spline[match(specieslist,species)],lwd=1.5, ylab='Relative Difference', xlab='Year (CE)')
polygon(c(seq(1600,1980,20),rev(seq(1600,1980,20))),c((ratiomeanupper),rev((ratiomeanlower))),col = alpha("grey75",.75), border = FALSE)
lines(seq(1600,1980,20),ratiomean,lwd=3)

abline(h=0,lty=2,lwd=2)
legend(1915, 5, legend=species,
       col=alpha(brewer.pal(n = length(species), name = "Set1"),0.8), lwd=2,cex=0.8,lty=spline) 
fig_label("C",region="plot", cex=2) 


par(fig = c(0.45,.6,0.75,.9), new = TRUE,mar = c(0,0,0,0),cex.lab=0.75, cex.axis=0.75,mgp=c(3,1,0))
...
par(mfrow=c(2,1),mar = c(1.5,5,2.5,2.5))
matplot(seq(1600,1980,by=20),t(sout.use[,18:37,1]),type='l',lty=1,lwd=.1,ylab="Survival Rate")
fig_label("A",region="plot", cex=2) 
par(mar = c(4.5,5,1.5,2.5))

matplot(seq(1600,1980,by=20),t(bout.use[,18:37,1]),type='l',lty=1,lwd=.1, ylim=c(0,3.7), xlab='Year (CE)',ylab="Establishment Rate")
fig_label("B",region="plot", cex=2) 
